...
classifier.add(Conv1D(filters=64, kernel_size=3, activation='relu', input_shape=(100, 1)))
...
"""卷积"""
threshold = 0.5
Y_predM1 = np.where(Y_predM1 > threshold, 1, 0)
...
"""卷积"""
threshold = 0.5
Y_predF1 = np.where(Y_predF1 > threshold, 1, 0)
...
for p in range(5):
    ...
    """卷积"""
    Y_predM2 = np.where(Y_predM2 > threshold, 1, 0)
    ...
    """卷积"""
    Y_predF2 = np.where(Y_predF2 > threshold, 1, 0)
    ...
    """卷积"""
    class_list=[]
    for i in imputed_list1:

        class_pred=classifier.predict(pad_sequences(i[:,1:], padding='post', maxlen=100))
        class_pred = np.where(class_pred > threshold, 1, 0)
    ...
    plt.scatter(x_axis_data7, y_axis_data7, c='lightblue',marker='h', alpha=0.1, linewidth=2)#, label='Most fair')
...
plt.scatter([mean(mostfairb)],[mean(mostfaira)], c='purple', marker='D',alpha=1, linewidth=5, label='Most fair')
plt.scatter([mean(onlyvarb)],[mean(onlyvara)], c='blue', marker='d', alpha=1, linewidth=5, label='Only variance')
plt.scatter([mean(randomb)],[mean(randoma)], c='black', marker='s', alpha=1, linewidth=5, label='random choose')
plt.scatter([mean(mostaccb)],[mean(mostacca)], c='green',  marker='o',alpha=1, linewidth=5, label='Most acc')
...
plt.scatter([mean(protectb)],[mean(protecta)], c='olive',marker='P', alpha=1, linewidth=5)#, label='Most fair')
